...
cargo run --release
```
4. **Try it out!** While the application is running, try resizing any open window using the `Ctrl+Alt+F9` hotkey. Enter any two letters from the grid to select the desired area corners. Application remains open and available to use on any window until finished in the terminal through `Ctrl+C`.
