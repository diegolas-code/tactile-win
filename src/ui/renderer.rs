...
use tiny_skia::{
    Color, FillRule, Paint, PathBuilder, Pixmap, Rect as SkiaRect, Stroke, Transform
};

use crate::domain::core::Rect;
use crate::domain::grid::Grid;
use crate::domain::keyboard::{QwertyLayout, GridCoords};
...
impl GridRenderer {
    ...
    fn render_letters(&mut self, pixmap: &mut Pixmap, letters: &[LetterPosition]) -> Result<(), RendererError> {
    ...
    fn render_single_letter(&mut self, pixmap: &mut Pixmap, letter_pos: &LetterPosition) -> Result<(), RendererError> {
        ...
        let mut path_builder = PathBuilder::new();
        if let Some(circle_rect) = SkiaRect::from_xywh(
            letter_pos.x - radius,
            letter_pos.y - radius,
            radius * 2.0,
            radius * 2.0,
...
mod tests {
    ...
    fn grid_layout_creation() {
        let grid = Grid::new(2, 3, Rect { x: 0, y: 0, w: 1920, h: 1080 }).unwrap();
        let canvas_rect = Rect { x: 0, y: 0, w: 1920, h: 1080 };
    ...
    #[test]
    fn dpi_scaling() {
        let grid = Grid::new(2, 2, Rect { x: 0, y: 0, w: 1920, h: 1080 }).unwrap();
        let canvas_rect = Rect { x: 0, y: 0, w: 1920, h: 1080 };
        ...
        if let (Some(normal_letter), Some(scaled_letter)) = (
            ...
            assert!(scaled_letter.font_size > normal_letter.font_size, "Scaled fonts should be larger");
        }
    }
    ...
    fn render_simple_layout() {
        ...
        let grid = Grid::new(2, 2, Rect { x: 0, y: 0, w: 1000, h: 800 }).unwrap();
        let canvas_rect = Rect { x: 0, y: 0, w: 1000, h: 800 };
